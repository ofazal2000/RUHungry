...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < numCategories; i++) {
            ...
            for (int j = 0; j < numDishes; j++) {
                ...
                Dish newDish = new Dish(categoryVar[i], dishName, ingredientIDs);
                MenuNode newNode = new MenuNode(newDish, currentHead);
                currentHead = newNode;
            }
    
            menuVar[i] = currentHead;
        }
    }
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        int bucketIndex = ingredientID % stockVarSize;
        if (stockVar[bucketIndex] != null) {
            newNode.setNextStockNode(stockVar[bucketIndex]);
        }
        stockVar[bucketIndex] = newNode;
    }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        StockNode targetStockNode;
        if (ingredientName == null) {
            targetStockNode = findStockNode(ingredientID);
        } else if (ingredientID == -1) {
            targetStockNode = findStockNode(ingredientName);
        } else {
            return; // Invalid parameters, do nothing
        }

        Ingredient targetIngredient = targetStockNode.getIngredient();
        int updatedStockLevel = targetIngredient.getStockLevel() + stockAmountToAdd;
        targetIngredient.setStockLevel(updatedStockLevel);
    }
    ...
    public void updatePriceAndProfit() {
        ...
        for (MenuNode categoryHead : menuVar) {
            MenuNode currentNode = categoryHead;
            while (currentNode != null) {
                Dish currentDish = currentNode.getDish();
                double ingredientCostTotal = 0;
                for (int ingredientID : currentDish.getStockID()) {
                    StockNode ingredientNode = findStockNode(ingredientID);
                    ingredientCostTotal += ingredientNode.getIngredient().getCost();
                }
                double dishPrice = ingredientCostTotal * 1.2;
                currentDish.setPriceOfDish(dishPrice);
                currentDish.setProfit(dishPrice - ingredientCostTotal);
    ...
    public void createStockHashTable (String inputFile){
        ...
        while (!StdIn.isEmpty()) {
            ...
            StdIn.readChar(); // Consume the space
            String ingredientName = StdIn.readLine();
            double ingredientCost = StdIn.readDouble();
            int ingredientStockLevel = StdIn.readInt();
    
            Ingredient newIngredient = new Ingredient(ingredientID, ingredientName, ingredientStockLevel, ingredientCost);
            StockNode newStockNode = new StockNode(newIngredient, null);
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode newTransactionNode = new TransactionNode(data, null);
        if (transactionVar == null) {
            transactionVar = newTransactionNode;
        } else {
            TransactionNode current = transactionVar;
            while (current.getNext() != null) {
                current = current.getNext();
            }
            current.setNext(newTransactionNode);
        }
    }
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        MenuNode foundMenuNode = findDish(dishName);
        Dish targetDish = foundMenuNode.getDish();
        int[] ingredientIDs = targetDish.getStockID();
    
        for (int ingredientID : ingredientIDs) {
            StockNode correspondingStockNode = findStockNode(ingredientID);
            if (correspondingStockNode.getIngredient().getStockLevel() < numberOfDishes) {
                return false;
            }
        }
    
        return true; // The dish is available
    }
    ...
    public void order (String dishName, int quantity){
        ...
        Dish requestedDish = findDish(dishName).getDish();
        if (checkDishAvailability(dishName, quantity) == true) {
            double profitFromOrder = requestedDish.getProfit() * quantity;
            addTransactionNode(new TransactionData("order", dishName, quantity, profitFromOrder, true));
    
            for (int id : requestedDish.getStockID()) {
                updateStock(null, id, -quantity);
            }
        } else {
            addTransactionNode(new TransactionData("order", dishName, quantity, 0, false));
    
            MenuNode menuPointer = findDish(dishName);
            MenuNode nextNode = menuPointer.getNextMenuNode();
            while (nextNode != null && nextNode != menuPointer) {
                Dish nextDish = nextNode.getDish();
                if (checkDishAvailability(nextDish.getName(), quantity) == true) {
                    order(nextDish.getName(), quantity);
                    return;
                } else {
                    addTransactionNode(new TransactionData("order", nextDish.getName(), quantity, 0, false));
                    nextNode = nextNode.getNextMenuNode();
                }
            }
    
            MenuNode categoryHead = menuVar[findCategoryIndex(requestedDish.getCategory())];
            while (categoryHead.getDish() != requestedDish) {
                if (checkDishAvailability(categoryHead.getDish().getName(), quantity) == true) {
                    order(categoryHead.getDish().getName(), quantity);
                    return;
                }
                addTransactionNode(new TransactionData("order", categoryHead.getDish().getName(), quantity, 0, false));
                categoryHead = categoryHead.getNextMenuNode();
            }
        }
    }
    ...
    public double profit () {
        ...
        TransactionNode currentNode = transactionVar;
        double totalProfit = 0;
        while (currentNode != null) {
            if (currentNode.getData().getSuccess()) {
                totalProfit += currentNode.getData().getProfit();
            }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        boolean canDonate = node.getIngredient().getStockLevel() >= quantity && profit > 50;
        addTransactionNode(new TransactionData("donation", ingredientName, quantity, 0, canDonate));
    ...
    public void restock (String ingredientName, int quantity){
        ...
        StockNode node = findStockNode(ingredientName);
        double costToRestock = node.getIngredient().getCost() * quantity;
        double profitMargin = profit();

        boolean isRestockSuccessful = profitMargin >= costToRestock;
        if (isRestockSuccessful) {
            updateStock(ingredientName, -1, quantity);
            profitMargin -= costToRestock;
        }
        addTransactionNode(new TransactionData("restock", ingredientName, quantity, isRestockSuccessful ? -costToRestock : 0, isRestockSuccessful));
    }
